...
/*      18.09.07 unity sie nie respawnuja                       
...
event OnLoadLevel(mission m, int n) {
    ...
    if (m.GetMissionNum() == GetMissionNum()) {
        nTownLoaded = true;
        if (GetCampaign().IsPartyEnemy(PARTY_NPC,ePartyPlayer1) || anBanishedHero[0]) { // 12.09.07
            ResetHeroIsCriminal(0); // 5.09.07
            SetHeroPartyNeutral(0); // 5.09.07
            anHeroCrimePoints[0] = eCrimePointsMurder; // 5.09.07
            anHeroCurrentCrime[0] = eCrimePointsMurder; // 5.09.07
        }
    }
