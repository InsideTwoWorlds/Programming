...
function int GetMultiplayerMissionAndPositionOffsets(int nCol, int nRow, int nLayer) {
    ...
    if (!GetMultiSinglePlayerMission(nStartCol,nStartRow)) return eNoMission;
...
function string MultiMissionName2SingleMissionName(string strMission, int nX, int nY) {
    ...
    GetMultiSinglePlayerMission(nCol,nRow);
...
consts {
    ...
    eQuestKillFirst        = 201;
    eQuestKillLast         = 260;
    eQuestKillUndeadFirst  = 101;
    eQuestKillUndeadLast   = 130;
    eQuestBringObjectFirst = 1;
    eQuestBringObjectLast  = 30;
...
function int CanAddQuest(int nQuestNum) {
    ...
    string strQuestsRange;
    int arrQuestsRange[];
    int nIndex;
    nQuestNum += eFirstQuest;
    GetMultiMissionInfoAttrib("QuestsRange", strQuestsRange);
    if (strQuestsRange.GetLength())
    {
        ScanIntValues(strQuestsRange, arrQuestsRange);
        for (nIndex = 0; nIndex < arrQuestsRange.GetSize(); nIndex += 2)
        {
            if (nQuestNum >= arrQuestsRange[nIndex] && nQuestNum <= arrQuestsRange[nIndex + 1]) return true;
        }
        return false;
    }
    return true;
...
function void GenerateEnemies() {
    ...
    GetMultiMissionInfoAttrib("MarkerEnemyConfig", strMarkerEnemyConfig);
    for (i = 0; i < astrMarkersEnemies.GetSize(); i++) {
        for (nMission = 0; nMission < GetCampaign().GetMissionsCnt(); nMission++) {
            ...
            for (j = 1; j <= n; j++) {
                ...
                strMarker = GetMarkerEnemyMulti(strMarkerEnemyConfig,astrMarkersEnemies[i],nEnemyType);
...
function int IsBringObjectLocationEmpty(int nLocation) {
    ...
    pMission.SearchObjectsInArea(anBOLPosX[nLocation],anBOLPosY[nLocation],eActionObjectCreateRangeA * 2,astrBOLName[nLocation]);
...
function void ActivateNextMap(unit uHero) {
    ...
    // unlockowanie miast
    GetMultiMissionInfoAttrib("UnlockTown", strValue);
    if (strValue.GetLength()) uHero.SetPlayersCommonAttribute(strValue,true);
...
#ifdef QUESTS_MULTI_DEBUG
        TRACE("ChooseQuestValues returned false: nType %d nGiverMarker %d nQuest %d nQuestMarker %d             \n",nType,nGiverMarker,nQuest,nQuestMarker);
#endif
        return false;
    }
    return true;
...
event CreatedNewNetworkPlayer(int nPlayerNum, int nMissionNum, int nX, int nY) {
    ...
    for (i = 0; i < eQuestsNum; i++) 
        if (anQuestState[i] == eQuestStateEnabled) {
            EnableQuest(i,GetHero(nPlayerNum));
            if (anQuestTakenFlag[i]) TakeQuest(i, GetHero(nPlayerNum));
        }
    return true;
